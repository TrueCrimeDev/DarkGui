...
class DarkApp {
    ...
    ThemeChanged(*) {
        ...
        if selected
            _Dark.DarkController.SetTheme(selected)
    }
}

class _Dark extends Gui {
    ; Use static properties instead of global variables
    Static OriginalWindowProc := 0
    Static DarkController := _DarkController()
    
    ; Control class extensions
    Static DarkCtl := _DarkCtl()
    Static Text := _Text()
    Static Button := _Button()
    Static Edit := _Edit()
    Static ListView := _ListView()
    Static ComboBox := _ComboBox()
    Static CheckBox := _CheckBox()
    Static Radio := _Radio()
    Static GroupBox := _GroupBox()
    Static ListBox := _ListBox()
    ...
    __New(Options := "", Title := A_ScriptName, EventObj?) {
        ...
        this.BackColor := this.DarkController.Theme["Background"]
        ...
        if (!_Dark.OriginalWindowProc) {
            ...
            WindowProc := CallbackCreate(this.DarkController.ProcessWindowMessage.Bind(this.DarkController), "Fast")
    ...
    ProcessCustomDraw(lParam) {
        NMCD := this.CreateNMCUSTOMDRAWTemplate()
...
class _Text extends Gui.Text {
    ...
    ApplyDarkTheme() {
        this.Opt("c" . Format("{:X}", _Dark.DarkController.Theme["Font"]))
        this.Redraw()
    }
}
...
class _Button extends Gui.Button {
    ...
    ApplyDarkTheme() {
        ...
        this.SetFont("c" . Format("{:X}", _Dark.DarkController.Theme["Font"]))
        _Dark.DarkController.ButtonColors[this.Hwnd] := Map(
            "bg", _Dark.DarkController.Theme["Controls"],
            "text", _Dark.DarkController.Theme["Font"]
        )
        this.Redraw()
    }
}
...
class _ListView extends Gui.ListView {
    ...
    ApplyDarkTheme() {
        ...
        SendMessage(LVM_SETTEXTCOLOR, 0, _Dark.DarkController.Theme["Font"], this.Hwnd)
        SendMessage(LVM_SETBKCOLOR, 0, _Dark.DarkController.Theme["Background"], this.Hwnd)
        SendMessage(LVM_SETTEXTBKCOLOR, 0, _Dark.DarkController.Theme["Background"], this.Hwnd)
    ...
    SetupCustomDraw() {
        ...
        LV_CustomDraw(ctrl, lParam) {
            ...
            if (dwDrawStage == CDDS_ITEMPREPAINT) {
                ...
                DllCall("gdi32\SetTextColor", "Ptr", hdc, "UInt", _Dark.DarkController.Theme["Font"])
...
class _ComboBox extends Gui.ComboBox {
    ...
    ApplyDarkTheme() {
        ...
        this.SetFont("c" . Format("{:X}", _Dark.DarkController.Theme["Font"]))
        _Dark.DarkController.ComboBoxes[this.Hwnd] := true
        this.Redraw()
    }
}
...
class _CheckBox extends Gui.CheckBox {
    ...
    ApplyDarkTheme() {
        ...
        this.SetFont("c" . Format("{:X}", _Dark.DarkController.Theme["Font"]))
        _Dark.DarkController.ButtonColors[this.Hwnd] := Map(
            "bg", _Dark.DarkController.Theme["Background"],
            "text", _Dark.DarkController.Theme["Font"]
        )
        this.Redraw()
    }
...
class _Radio extends Gui.Radio {
    ...
    ApplyDarkTheme() {
        ...
        _Dark.DarkController.RadioButtons[this.Hwnd] := true
        this.Redraw()
    }
}
...
class _GroupBox extends Gui.GroupBox {
    ...
    ApplyDarkTheme() {
        DllCall("uxtheme\SetWindowTheme", "Ptr", this.hWnd, "Str", "", "Ptr", 0)
        this.SetFont("c" . Format("{:X}", _Dark.DarkController.Theme["Font"]))
        _Dark.DarkController.GroupBoxes[this.Hwnd] := true
    ...
    PaintGroupBox(wParam, lParam, msg, hwnd) {
        ...
        borderColor := _Dark.DarkController.Theme["Font"]
        hPen := DllCall("CreatePen", "Int", PS_SOLID, "Int", 1, "UInt", borderColor, "Ptr")
        DllCall("SelectObject", "Ptr", hdc, "Ptr", hPen)

        ; Set text color and background
        DllCall("SetTextColor", "Ptr", hdc, "UInt", _Dark.DarkController.Theme["Font"])
        DllCall("SetBkColor", "Ptr", hdc, "UInt", _Dark.DarkController.Theme["Background"])
        ...
        textRect := this.BufferFromRect(textX, textY, textX + textWidth, textY + textHeight)
...
class _ListBox extends Gui.ListBox {
    ...
    ApplyDarkTheme() {
        DllCall("uxtheme\SetWindowTheme", "Ptr", this.hWnd, "Str", "DarkMode_Explorer", "Ptr", 0)
        this.SetFont("c" . Format("{:X}", _Dark.DarkController.Theme["Font"]))
        _Dark.DarkController.ListBoxControls[this.Hwnd] := true
        this.Redraw()
    }
}
...
class _Slider extends Gui.Slider {
    ...
    ApplyDarkTheme() {
        DllCall("uxtheme\SetWindowTheme", "Ptr", this.hWnd, "Str", "DarkMode_Explorer", "Ptr", 0)
        _Dark.DarkController.SliderControls[this.Hwnd] := true
        this.Redraw()
    }
}
...
class _Progress extends Gui.Progress {
    ...
    ApplyDarkTheme() {
        DllCall("uxtheme\SetWindowTheme", "Ptr", this.hWnd, "Str", "DarkMode_Explorer", "Ptr", 0)
        _Dark.DarkController.ProgressControls[this.Hwnd] := true
        this.Redraw()
    }
}
...
class _DateTime extends Gui.DateTime {
    ...
    ApplyDarkTheme() {
        DllCall("uxtheme\SetWindowTheme", "Ptr", this.hWnd, "Str", "DarkMode_Explorer", "Ptr", 0)
        this.SetFont("c" . Format("{:X}", _Dark.DarkController.Theme["Font"]))
        _Dark.DarkController.DateTimeControls[this.Hwnd] := true
        this.Redraw()
    }
}
...
class _Tab extends Gui.Tab {
    ...
    ApplyDarkTheme() {
        DllCall("uxtheme\SetWindowTheme", "Ptr", this.hWnd, "Str", "DarkMode_Explorer", "Ptr", 0)
        this.SetFont("c" . Format("{:X}", _Dark.DarkController.Theme["Font"]))
        _Dark.DarkController.TabControls[this.Hwnd] := true
        this.Redraw()
    }
}

class _DarkController {
   ...
   ; Initialize all the GUI control extensions
   for ctl in [_DarkCtl, _Text, _Button, _Edit, _ListView, _ComboBox, 
              _CheckBox, _Radio, _GroupBox, _ListBox, _Slider, 
              _Progress, _DateTime, _Tab] {
       try { 
           ctl.__New.Call()
       } catch {
           ; In case of any error, continue without breaking initialization
       }
   }
